    ...
    <b-card class="card-margin" v-if="statusCompleted">
      <div class="scroll-table">
        <table>
          ...
          >
            <td
              v-if="race.position == 'NC'"
              class="result-last-race-td result-last-race-position"
            >
              DNF
            </td>
            <td v-else class="result-last-race-td result-last-race-position">
              {{ race.position }}
            </td>
            <td v-if="race.position == 'NC'" class="result-last-race-td">
              <img
                :src="getSymbolImage(race.grid_position, 20)"
                :alt="'Symbol'"
                :class="getSymbolClass(race.grid_position, 20)"
              />
              <span class="position-difference">
                {{
                  race.grid_position - 20 != 0
                    ? Math.abs(race.grid_position - 20)
                    : ''
                }}
              </span>
            </td>
            <td v-else class="result-last-race-td">
              <img
            ...
            <td class="result-last-race-td pointer">
              <a
                v-on:click="
                  getDriverObject(
                    createId(`${race.first_name} ${race.last_name}`)
                  )
                "
                class="table-link-name"
                >{{ race.first_name }} {{ race.last_name }}</a
              >
            </td>
            <td
            ...
            <td class="result-last-race-td pointer">
              <a
                v-on:click="getConstructorObject(createId(race.team))"
